...
```{r}
set.seed(123)
# Determine how many Lake Trout and SMB fish
fish_counts <- fish_clean %>%
  group_by(species, fishNum) %>%
  summarise(n_obs = n(), .groups = "drop_last") %>%
  ungroup()

# Select equal number of fish from each species 
sampled_fish <- fish_counts %>%
  group_by(species) %>%
  slice_sample(n = 10)

# Then use fishNum from this table to extract the corresponding observations
balanced_fish_data <- fish_clean %>%
  filter(fishNum %in% sampled_fish$fishNum)
balanced_freq_data <- freq_clean %>%
  filter(fishNum %in% sampled_fish$fishNum)
```
...
# Execute data split
split_results <- split_data(balanced_fish_data)
...
# Execute data split
split_results <- split_data(balanced_freq_data)
train_data <- split_results$train_data
test_data  <- split_results$test_data
...
rf_result_all_freq <- train_random_forest(X2_train, Y2_train, X2_test, Y2_test)
```

```{r}
# 3. PCA Frequencies response
# Execute data split
pca_freq <- balanced_freq_data  %>% 
  select(fishNum, species, any_of(paste0("F", seq(63, 77, by = 0.5))))
split_results <- split_data(pca_freq)
train_data <- split_results$train_data
test_data  <- split_results$test_data
...
```{r}
rf_result_pca_freq <- train_random_forest(X3_train, Y3_train, X3_test, Y3_test)
