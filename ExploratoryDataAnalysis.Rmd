...
source('AnalysisScripts/read_tidy_export_EVfiles.R')
```
...
```{r}
# Create Frequency only dataset for easy to analysis later
freq_only <- fish_data %>%
...
```{r}
# 'fish_data` is raw and `fish_clean` is cleaned
summary_table <- tibble(
  Stage = c("Before Cleaning", "After Cleaning","Frequency Only"),
  Num_Rows = c(nrow(processed_data), nrow(fish_clean), nrow(freq_clean)),
  Num_Columns = c(ncol(processed_data), ncol(fish_clean), ncol(freq_clean))
)

kable(summary_table, caption = "Dataset Size Before and After Cleaning")
```
...
```{r}
# Count unique fish per species
species_counts <- fish_clean %>%
  select(fishNum, species) %>%
  distinct() %>%
  count(species)

# Plot count of unique fish
ggplot(species_counts, aes(x = species, y = n, fill = species)) +
  geom_col(width = 0.6) +
  labs(
    title = "Number of Unique Fish per Species",
    x = "Species",
    y = "Count of Unique Fish"
  ) +
  theme_minimal() +
...
# Plot mean frequency strength per fish colored by species
ggplot() +
...
#'
#' @description
#' Computes the mean of a numeric vector along with the corresponding confidence 
#' interval calculated via the t-distribution. Missing values are excluded from the 
#' computation. If there are fewer than two valid observations, 
#' NA is returned for the bounds.
#'
#' @param x A numeric vector.
#' @param confidence A numeric value indicating the desired confidence level (default is 0.95).
#'
#' @return A tibble with columns for the mean, lower, and upper bounds of the confidence interval.
...
```{r}
#' Plot Confidence Intervals for a Fish's Frequency Response
#'
#' @description
#' This function filters the data for a given fish ID, reshapes the data to long format,
#' computes the mean strength and its 95% confidence intervals per frequency,
#' and returns a ggplot of the results.
#'
#' @param fish_id A character string representing the fish's ID.
#'
#' @return A ggplot object showing mean strength with a confidence interval ribbon.
...
```{r}
#' Identify Highly Correlated Frequencies for a Given Species
#'
#' @description
#' This function filters the provided dataset for a specific species, extracts the frequency response
#' columns (from 45 to 260 kHz, in 0.5 kHz steps), computes the correlation matrix using pairwise complete
#' observations, and returns a long-format tibble of frequency pairs with an absolute correlation greater than
#' the specified threshold.
#'
#' @param data A tibble or data frame that contains a column called `species` and frequency response columns
#' with an "F" prefix (e.g., F45, F45.5, ...).
#' @param species_name A character string indicating the species for which to compute the frequency correlations.
#' @param threshold A numeric value (default = 0.7). Only frequency pairs with an absolute correlation greater than
#' this threshold will be returned.
#'
#' @return A tibble containing columns for the frequency pairs and their correlation.
...
```{r}
#' Compute Correlations of Biological Features with Frequencies for a Given Species
#'
#' @description
#' This function filters the global dataset for a specified species, converts the `sex` column
#' to numeric, selects relevant biological features along with frequency response columns, computes
#' the correlation matrix using pairwise complete observations, and reshapes the matrix to a long
#' format focusing on correlations between biological features and frequencies.
#'
#' @param species_name A character string specifying the species (e.g., "lakeTrout" or "smallmouthBass").
#'
#' @return A tibble containing the correlations between selected biological features and frequency response variables,
#' with an added column for the species name.
...
ggplot(species_corr, aes(x = Frequency_Bin, 
                         y = Variable1, 
  ...
  labs(
    title = "Species-Specific Correlation Between Biological 
    Features and Frequency Response",
  ...
  theme_minimal() +
  theme(
    # Adjust title size, bold, and center
...
Sex-Comparsion is not focus on the project objective so I remove it. 
